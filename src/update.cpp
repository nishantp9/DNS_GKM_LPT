...
void PrintStats(ptype *W[5], UBspline_3d_d *splinep[8])
{
	...
	SKEWu = DUDX3 / pow(DUDX2, 1.5);
	SKEWv = DVDY3 / pow(DVDY2, 1.5);
	SKEWw = DWDZ3 / pow(DWDZ2, 1.5);
	KURTu = DUDX4 / pow(DUDX2, 2);
	KURTv = DVDY4 / pow(DVDY2, 2);
	KURTw = DWDZ4 / pow(DWDZ2, 2);
	...
	if (myrank_3d == 0)
	{
		std::cout << scientific << t / t0 << " " << TKE << " " << Diss << " " << SijSij << " " << SKEWu << " " << SKEWv << " " << SKEWw << " " << KURTu << " " << KURTv << " " << KURTw << " "
				  << Prms << " " << Trms << " "
				  << " " << part_TKE << " "
				  << " " << part_SijSij << " " << part_SKEWu << " " << part_SKEWw << std::endl;
	}
}
