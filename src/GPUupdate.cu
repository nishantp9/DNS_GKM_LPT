...

{
	...
	if (TAG == 1)
	{
		bound_check = (ix < d_nt_segx - 3 && iy < d_nt_segy - 3 && iz < d_nt_segz - 3 && ix > 1 && iy > 2 && iz > 2);
	}
	else if (TAG == 2)
	{
		bound_check = (ix < d_nt_segx - 3 && iy < d_nt_segy - 3 && iz < d_nt_segz - 3 && ix > 2 && iy > 1 && iz > 2);
	}
	else if (TAG == 3)
	{
		bound_check = (ix < d_nt_segx - 3 && iy < d_nt_segy - 3 && iz < d_nt_segz - 3 && ix > 2 && iy > 2 && iz > 1);
	}
	...
	if (bound_check)
	{
		 ...
		 * -------------------------------------------------------------------------*/
		gm0 = d_dt - tau * (1 - exp(-d_dt / tau));
		gm1 = -(1 - exp(-d_dt / tau)) / gm0;
		gm2 = (-d_dt + 2 * tau * (1 - exp(-d_dt / tau)) - d_dt * exp(-d_dt / tau)) / gm0;
		gm3 = -gm1;
		gm4 = (d_dt * exp(-d_dt / tau) - tau * (1 - exp(-d_dt / tau))) / gm0;
		gm5 = tau * gm3;
...
__device__ void d_slopesolver(ptype b[5], ptype U[3], ptype lam, ptype a[5])
{
	...
	a[0] = b[0] - a[1] * U[0] - a[2] * U[1] - a[3] * U[2] - .5 * a[4] * (U[0] * U[0] + U[1] * U[1] + U[2] * U[2] + (d_K + 3) / (2 * lam));
}
...
__device__ void d_MCal(ptype M[5], ptype I[3][7], ptype If[3][7], ptype Ie2, ptype Ie4, int k, int l, int m, ptype ax[5])
{
	...
	M[4] = 0.25 * ax[4] * (I[0][4 + k] * If[1][l] * If[2][m] + I[0][k] * If[1][4 + l] * If[2][m] + I[0][k] * If[1][l] * If[2][4 + m] + I[0][k] * If[1][l] * If[2][m] * Ie4 + 2 * I[0][2 + k] * If[1][2 + l] * If[2][m] + 2 * I[0][2 + k] * If[1][l] * If[2][2 + m] + 2 * I[0][2 + k] * If[1][l] * If[2][m] * Ie2 + 2 * I[0][k] * If[1][2 + l] * If[2][2 + m] + 2 * I[0][k] * If[1][2 + l] * If[2][m] * Ie2 + 2 * I[0][k] * If[1][l] * If[2][2 + m] * Ie2) +
