		  ...
		  ptype DWyl[5], ptype DWyr[5], ptype DWzl[5], ptype DWzr[5], ptype tau, ptype dt, ptype dx, ptype F[5])
{
	 ...
	 * -------------------------------------------------------------------------*/
	gm0 = dt - tau * (1 - exp(-dt / tau));
	gm1 = -(1 - exp(-dt / tau)) / gm0;
	gm2 = (-dt + 2 * tau * (1 - exp(-dt / tau)) - dt * exp(-dt / tau)) / gm0;
	gm3 = -gm1;
	gm4 = (dt * exp(-dt / tau) - tau * (1 - exp(-dt / tau))) / gm0;
	gm5 = tau * gm3;
...
void slopesolver(ptype b[5], ptype U[3], ptype lam, ptype a[5])
{
	...
	a[0] = b[0] - a[1] * U[0] - a[2] * U[1] - a[3] * U[2] - .5 * a[4] * (U[0] * U[0] + U[1] * U[1] + U[2] * U[2] + (K + 3) / (2 * lam));
}
